...
#_(letfn [(seq-of-maps?
          ...
          (combine-keys
            [[suffix m]]
            (into {} (for [[k v] m] [(keyword (str (name k) \- (name suffix))) v])))
          (concat-raise
            [m ks]
    ...
    (->> "../../Documents/ch-products-elite-map/config/Custom.binds"
         ...
         second
         (group-by category)
         (#(update % :misc (partial into {})))
         (#(update % :settings (comp (partial into {}) flatten-settings)))
         (#(update % :axes (comp values-into-map (partial group-by device) flatten-axes)))
         (#(update % :commands (comp values-into-map (partial group-by device) flatten-commands)))
